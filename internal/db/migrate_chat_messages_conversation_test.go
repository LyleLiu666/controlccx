...
func TestMigrate_ChatMessages_AddConversationIDBackfillAndIndex(t *testing.T) {
	...
	if _, err := conn.ExecContext(ctx, `
		...
		INSERT INTO chat_messages(ts, role, content) VALUES (1001, 'user', 'legacy');
	`); err != nil {
	...
	rows, err := conn.QueryContext(ctx, "PRAGMA table_info(chat_messages);")
	if err != nil {
	...
	if err := conn.QueryRowContext(ctx, `
		...
		WHERE type='index' AND name='idx_chat_messages_conversation_id_id';
	`).Scan(&indexName); err != nil {
		t.Fatalf("expected idx_chat_messages_conversation_id_id index: %v", err)
	}
}
