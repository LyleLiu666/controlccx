...
func buildSystemPrompt() string {
	...
	b.WriteString("4) worker 选择建议：简单且追求速度 -> claude-code；严肃/生产级迭代 -> codex；不确定则先问再提。\n")
	b.WriteString("5) exec 仅可在用户明确要求 shell/脚本执行时使用；自动推荐不使用 exec。\n")
	b.WriteString("6) 对写操作工具（如创建任务）若必填参数缺失，必须先向用户索取，禁止猜测/自动补全。\n")
	b.WriteString("7) 写操作前先给执行摘要（目标/worker/验收），得到确认后再提交。\n")
	b.WriteString("8) 高风险动作必须遵守工具参数约束（例如 enter-unsafe 需要 confirm=true）。\n\n")
	b.WriteString("输出约束：\n")
