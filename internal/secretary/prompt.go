...
func buildSystemPrompt() string {
	...
	b.WriteString("当你需要调用工具时，你必须只输出一个 XML 块，且不能输出任何其他文本：\n")
	b.WriteString("<tool_data>\n")
	b.WriteString("  <call>\n")
	b.WriteString("    <tool_name>TOOL_NAME</tool_name>\n")
	b.WriteString("    <!-- 工具参数：每个参数一个 XML 字段 -->\n")
	b.WriteString("    <arg1>...</arg1>\n")
	b.WriteString("  </call>\n")
	b.WriteString("</tool_data>\n\n")
	b.WriteString("参数格式约定：\n")
	b.WriteString("- 每个参数一个 XML 字段；字段名必须是该工具参数名；不要把参数写进 XML 属性。\n")
	b.WriteString("- 只传该工具支持的参数字段；不要自造元信息字段（例如把“成功/失败”写成 <status>...）。\n")
	b.WriteString("- <status> 不是通用字段：只有当工具参数列表明确包含 status 时才传。\n")
	b.WriteString("- 布尔值用 true/false；数字直接写数字。\n")
	b.WriteString("- JSON 仅允许作为字符串放进工具声明的 *_json 字段里（例如 tool_fields_json）；严禁用 JSON 结构进行工具调用。\n")
	b.WriteString("- 如果某个工具的参数列表里包含 [runopts]，表示它支持安全参数组；这些参数需要作为独立字段直接传入（不要包一层 <runopts>）。\n")
	b.WriteString("  可用 [runopts] 字段：" + strings.Join(sectools.RunOptsParams, "、") + "\n\n")
	b.WriteString("工具调用必须使用 <tool_data> XML。严禁使用 JSON 工具调用。\n")
	b.WriteString("工具结果会以 <tool_result> XML 发给你：禁止原样复述该 XML；请用结果继续下一次工具调用或直接回答。\n")
	b.WriteString("不要猜测工具名，只能从下面的可用工具列表里选择。\n\n")
	b.WriteString("工具说明：\n")
